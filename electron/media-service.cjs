...
class MediaService {
  constructor(app, catalog, getWindow) {
    ...
    const handlers = {
      create: event => {
        const prepared = this.prepared.get(event.sender);
        if (prepared) {
          this.prepared.delete(event.sender);
          return prepared;
        }
        return this.createSession(event.sender);
      },
      open: (event, id, mediaId, fps, startTime = 0, openToken = 0) => this.session(event, id).open(mediaId, number(fps, 1, 240), number(startTime, 0, 1e10), integer(openToken, 0, Number.MAX_SAFE_INTEGER)),
  ...
  prepare(owner) {
    if (this.closing || owner.isDestroyed() || this.prepared.has(owner)) return;
    const ready = this.createSession(owner);
    this.prepared.set(owner, ready);
    // Prewarming is optional: a failed attempt must not poison the next create.
    void ready.catch(() => {
      if (this.prepared.get(owner) === ready) this.prepared.delete(owner);
    });
  ...
  async createSession(owner) {
    if (this.closing || owner.isDestroyed()) throw new Error('Media session closed');
    if (this.sessions.size >= 4) throw new Error('媒体会话数量已达上限');
    startupTrace.mark('core.load-start');
    this.core ||= loadCore(this.app);
    startupTrace.mark('core.load-end');
    startupTrace.mark('session.construct-start');
    const session = new Session(this.core, owner, this.catalog);
    startupTrace.mark('session.construct-end');
    this.sessions.set(session.id, session);
    try {
      await session.ready;
      startupTrace.mark('session.ready');
      if (this.closing || session.closed || owner.isDestroyed()) throw new Error('Media session closed');
      return session.id;
    } catch (error) {
  ...
  async dispose() { this.closing = true; this.prepared.clear(); await Promise.all([...this.sessions.values()].map(s => s.destroy())); this.sessions.clear(); }
}
...
class Session {
  ...
  send(type, data = {}) { if (!this.closed && !this.owner.isDestroyed()) this.owner.send('media:state', { id: this.id, type, openToken: this.openToken, ...data }); }
  async open(mediaId, fps, startTime = 0, openToken = 0) {
    startupTrace.mark('media.open');
    const generation = this.generation = (this.generation || 0) + 1;
    this.loaded = false; this.metadataReady = false; this.framePresentable = false; this.pendingSeek = null;
  ...
  seek(seconds, mediaId) {
    ...
    if (!this.metadataReady) { this.pendingSeek = { seconds, generation: this.generation }; return; }
  ...
  events() {
    ...
    for (const event of this.player.pollEvents()) {
      ...
      if (event.type === 'file-loaded') {
        ...
        // Synchronous property reads here can block the main thread while mpv
        // waits for its first render callback (a 200 ms VO timeout). Service
        // rendering first; playback-restart is the barrier for reading metadata.
        this.loaded = true;
        this.time = this.startTime;
        this.queueFrame();
      }
      if (event.type === 'playback-restart' && this.loaded) {
        startupTrace.mark('media.playback-restart');
        const info = this.player.getInfo();
        this.framePresentable = true;
        this.seeked = true;
        if (!this.metadataReady) {
          startupTrace.mark('media.metadata-ready');
      ...
      if (event.type === 'video-reconfig' && this.metadataReady) {
        const info = this.player.getInfo();
  ...
  async pump() {
    while ((this.pending || this.exactFrames.length) && !this.closed && this.loaded) {
      ...
      try {
        ...
        if (this.mode === 'shared-texture') {
          ...
          startupTrace.mark('texture.render-start');
          const textureInfo = await renderPlayer.renderSharedTexture(width, height);
          startupTrace.mark('texture.render-end');
          if (textureInfo?.busy) {
            this.frameId -= 1;
          ...
          // A provisional render drives mpv before dimensions/first-frame
          // readiness are known. Never expose its blank or tiny render target.
          if (!presentable || generation !== this.generation || this.closed) {
            if (slotId !== null) renderPlayer.releaseSharedTexture(slotId);
            request?.reject(new Error('媒体已切换或尚未就绪'));
            continue;
          }
        ...
        if (generation !== this.generation || this.closed) {
          request?.reject(new Error('媒体已切换或关闭'));
          continue;
        }
